...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    logc = -torch.max(scores, 1)[0]
    e_scores = torch.exp(scores + logc.view(-1, 1))
    sum_scores = e_scores.sum(dim=1)
    final_scores = -torch.log(e_scores/sum_scores.view(-1, 1))
    loss = final_scores[range(N), y].sum()
    loss /= N
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    label_mask = torch.zeros_like(final_scores)
    label_mask[range(N), y] = 1
    dl_ds = torch.exp(scores)/torch.exp(scores).sum(dim=1).view(-1, 1) - label_mask
    dl_ds /= N
    gradW2 = h1.T.mm(dl_ds)
    gradW2 += 2 * reg * W2
    grads['W2'] = gradW2
    grads['b2'] = torch.sum(dl_ds, dim=0)
    ds_dh = dl_ds.mm(W2.T)
    ds_dh[h1 == 0] = 0
    gradW1 = X.T.mm(ds_dh)
    gradW1 += 2 * reg * W1
    grads['W1'] = gradW1
    grads['b1'] = torch.sum(ds_dh, dim=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [0.001, 0.01, 0.1, 1, 3]
  hidden_sizes = [4, 16, 64, 128, 256]
  regularization_strengths = [0.0001, 0.001, 0.01, 0.1, 1]
  learning_rate_decays = [1, 0.95, 0.9, 0.85, 0.5]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
    ...
    for h in hidden_sizes:
      for r in regularization_strengths:
        for ld in learning_rate_decays:
          ...
          stat = model.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'], l, ld, r, 3000, 1000)
          y_pred = model.predict(data_dict['X_val'])
          accuracy = torch.sum(data_dict['y_val'] == y_pred).item() / y_pred.shape[0]
