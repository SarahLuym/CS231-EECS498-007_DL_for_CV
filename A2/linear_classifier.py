...
def svm_loss_vectorized(W, X, y, reg):
  ...
  # idx[range(num_train), y] = 0
  # temp_x = idx * X
  mask_margin = margin.clone()
  mask_margin[mask_margin > 0] = 1
  mask_margin[mask_margin < 0] = 0
  mask_correct_y = torch.zeros_like(margin, dtype=torch.bool)
  mask_correct_y[range(num_train), y] = True
  mask_margin[mask_correct_y] = mask_margin.sum(dim=1) * -1
  dW = mask_margin.t().mm(X).t()
  dW /= num_train
  dW += 2*reg*W
  #############################################################################
  #                             END OF YOUR CODE                              #
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [0.0001, 0.001, 0.01, 0.03, 1]
  regularization_strengths = [0.01, 0.03, 0.1, 1, 5]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    score = W.t().mv(X[i])
    logc = -score.max()
    score = torch.exp(score + logc)
    sum_score = score.sum()
    score /= sum_score
    loss += -torch.log(score[y[i]])
    for j in range(num_class):
      if j == y[i]:
        dW[:, j] += X[i, :] * score[j] - X[i, :]
      else:
        dW[:, j] += X[i, :] * score[j]
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  scores = torch.exp(scores + logc.view(-1, 1))
  sum_score = scores.sum(dim=1)
  scores /= sum_score.reshape(-1, 1)
  loss = -torch.log(scores[range(num_train), y]).sum()
  label_mask = torch.zeros_like(scores)
  label_mask[range(num_train), y] = 1
  dW += X.T.mm(scores - label_mask)
