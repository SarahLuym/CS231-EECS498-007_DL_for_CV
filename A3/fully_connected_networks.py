...
class Linear(object):
  ...
  def forward(x, w, b):
    ...
    out = x.reshape(x.shape[0], -1).mm(w)+ b.reshape(1, -1)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, w, b)
  ...
  def backward(dout, cache):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    dx = dout.mm(cache[1].t()).reshape(x.shape)
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    hid_out, cache = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
    scores, cache_L = Linear.forward(hid_out, self.params['W2'], self.params['b2'])
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    dh, dw2, db2 = Linear.backward(dout, cache_L)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for n, hidden_dim in enumerate(hidden_dims[1:]):
      self.params['W{}'.format(n+2)] = weight_scale * torch.randn(hidden_dims[n], hidden_dims[n+1], dtype=dtype, device=device)
      self.params['b{}'.format(n+2)] = torch.zeros(hidden_dims[n+1], dtype=dtype, device=device)

    self.params['W{}'.format(self.num_layers)] = weight_scale * torch.randn(hidden_dims[-1], num_classes, dtype=dtype, device=device)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers-1):
      hidden_out, cache = Linear_ReLU.forward(previous, self.params['W{}'.format(i+1)], self.params['b{}'.format(i+1)])
      cache_dict['cache_LR{}'.format(i+1)] = cache
      hidden_out_dict['hidden_out_{}'.format(i+1)] = hidden_out
      previous = hidden_out
      if self.use_dropout:
        drop_out, cache = Dropout.forward(previous, self.dropout_param)
        cache_dict['cache_DR{}'.format(i+1)] = cache
        hidden_out_dict['drop_out_{}'.format(i+1)] = drop_out
        previous = drop_out
    
    scores, cache_L = Linear.forward(previous, self.params['W{}'.format(self.num_layers)], self.params['b{}'.format(self.num_layers)])
    cache_dict['cache_L'] = cache_L
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dh, dw, db = Linear.backward(dout, cache_dict['cache_L'])
    grads['W{}'.format(self.num_layers)] = dw + 2 * self.reg * self.params['W{}'.format(self.num_layers)]
    grads['b{}'.format(self.num_layers)] = db
    previous_out = dh
    for i in range(self.num_layers-1):
      if self.use_dropout:
        previous_out = Dropout.backward(previous_out, cache_dict['cache_DR{}'.format(self.num_layers - i - 1)])
      dh, dw, db = Linear_ReLU.backward(previous_out, cache_dict['cache_LR{}'.format(self.num_layers - i - 1)])
      grads['W{}'.format(self.num_layers - i - 1)] = dw + 2 * self.reg * self.params['W{}'.format(self.num_layers - i - 1)]
      grads['b{}'.format(self.num_layers - i - 1)] = db
      previous_out = dh
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
def create_solver_instance(data_dict, dtype, device):
  ...
  solver = None
  # Replace "pass" statement with your code
  solver = Solver(model, data_dict, num_epochs=100, device='cuda')
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return solver
...
def rmsprop(w, dw, config=None):
  ...
  w -= config['learning_rate'] * dw / (grad_squared.sqrt() + 1e-7)
  next_w = w
  ###########################################################################
  #                             END OF YOUR CODE                            #
  ###########################################################################
...
def adam(w, dw, config=None):
  ...
  config['m'] = moment1
  config['v'] = moment2
  w -= config['learning_rate'] * moment1_unbias / (moment2_unbias.sqrt() + 1e-7)
  next_w = w
  #############################################################################
  #                              END OF YOUR CODE                             #
  #############################################################################
