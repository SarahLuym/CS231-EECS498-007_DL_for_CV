...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    out = torch.zeros((N, F, H1, W1), dtype=x.dtype).to(x.device)
    for i in range(N):
      for j in range(F):
        for h in range(H1):
          for k in range(W1):
            out[i, j, h, k] = torch.sum(x_pad[i, :, h*stride: h*stride+HH, k*stride: k*stride+WW] * w[j]) + b[j]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, w, b, conv_param)
  ...
  def backward(dout, cache):
    ...
    for i in range(N):
      for j in range(F):
        for h in range(H1):
          for k in range(W1):
            dw[j] += x_pad[i, :, h*stride:h*stride+HH, k*stride:k*stride+WW] * dout[i, j, h, k]
...
class MaxPool(object):
  ...
  def forward(x, pool_param):
    ...
    out = torch.zeros((N, C, H1, W1), dtype=x.dtype).to(device=x.device)
    for i in range(N):
      for j in range(C):
        for h in range(H1):
          for k in range(W1):
            out[i, j, h, k] = torch.max(x[i, j, h*stride: h*stride+pool_height, k*stride: k*stride+pool_width])
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, pool_param)
  ...
  def backward(dout, cache):
    ...
    for i in range(N):
      for j in range(C):
        for h in range(H1):
          for k in range(W1):
            max_idx = torch.argmax(x[i, j, h*stride: h*stride+pool_height, k*stride: k*stride+pool_width])
            row = max_idx // pool_width
            column = max_idx % pool_width
            dx[i, j, h*stride+row, k*stride+column] += dout[i, j, h, k]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    self.params['W2'] = weight_scale * torch.randn((int(num_filters*H*W//4), hidden_dim), dtype=dtype, device=device)
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    inter_out, inter_cache = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
    hidden_out, hidden_cache = Linear_ReLU.forward(inter_out, W2, b2)
    ...
    dx1, grads['W3'], grads['b3'] = Linear.backward(d1, score_cache)
    grads['W3'] += 2 * self.reg * W3
    dx2, grads['W2'], grads['b2'] = Linear_ReLU.backward(dx1, hidden_cache)
    grads['W2'] += 2 * self.reg * W2
    dx3, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dx2, inter_cache)
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    if weight_scale == 'kaiming':
      self.params['W1'] = kaiming_initializer(C, num_filters[0], 3, device=device, dtype=dtype)
      ...
      final_size = int(num_filters[-1] * H * W / (2 ** (2 * len(max_pools))))
      self.params['W{}'.format(self.num_layers)] = kaiming_initializer(final_size, num_classes, relu=False, device=device, dtype=dtype)
      self.params['b{}'.format(self.num_layers)] = torch.zeros((num_classes,), device=device, dtype=dtype)
      
      for i in range(self.num_layers -2):
        self.params['W{}'.format(i+2)] = kaiming_initializer(num_filters[i], num_filters[i+1], 3, device=device, dtype=dtype)
        self.params['b{}'.format(i+2)] = torch.zeros((num_filters[i+1],), device=device, dtype=dtype)
        if self.batchnorm:
          self.params['gamma{}'.format(i+2)] = torch.ones((num_filters[i+1],), device=device, dtype=dtype)
          self.params['beta{}'.format(i+2)] = torch.zeros((num_filters[i+1],), device=device, dtype=dtype)
    else:
      self.params['W1'] = weight_scale * torch.randn((num_filters[0], C, 3, 3), device=device, dtype=dtype)
      self.params['b1'] = torch.zeros((num_filters[0],), device=device, dtype=dtype)
      if self.batchnorm:
        self.params['gamma1'] = torch.ones((num_filters[0],), device=device, dtype=dtype)
        self.params['beta1'] = torch.zeros((num_filters[0],), device=device, dtype=dtype)
      final_size = int(num_filters[-1] * H * W / (2 ** (2 * len(max_pools))))
      self.params['W{}'.format(self.num_layers)] = weight_scale * torch.randn((final_size, num_classes), device=device, dtype=dtype)
      self.params['b{}'.format(self.num_layers)] = torch.zeros((num_classes,), device=device, dtype=dtype)
      
      for i in range(self.num_layers -2):
        self.params['W{}'.format(i+2)] = weight_scale * torch.randn((num_filters[i+1], num_filters[i], 3, 3), device=device, dtype=dtype)
        self.params['b{}'.format(i+2)] = torch.zeros((num_filters[i+1],), device=device, dtype=dtype)
        if self.batchnorm:
          self.params['gamma{}'.format(i+2)] = torch.ones((num_filters[i+1],), device=device, dtype=dtype)
          self.params['beta{}'.format(i+2)] = torch.zeros((num_filters[i+1],), device=device, dtype=dtype)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1):
      if i in self.max_pools:
        if self.batchnorm:
          hidden_out, hidden_cache = Conv_BatchNorm_ReLU_Pool.forward(hidden_out, self.params['W{}'.format(i+1)], self.params['b{}'.format(i+1)], self.params['gamma{}'.format(i+1)], self.params['beta{}'.format(i+1)], conv_param, self.bn_params[i], pool_param)
        else:
          hidden_out, hidden_cache = Conv_ReLU_Pool.forward(hidden_out, self.params['W{}'.format(i+1)], self.params['b{}'.format(i+1)], conv_param, pool_param)
      elif self.batchnorm:
        hidden_out, hidden_cache = Conv_BatchNorm_ReLU.forward(hidden_out, self.params['W{}'.format(i+1)], self.params['b{}'.format(i+1)], self.params['gamma{}'.format(i+1)], self.params['beta{}'.format(i+1)], conv_param, self.bn_params[i])
      else:
        hidden_out, hidden_cache = Conv_ReLU.forward(hidden_out, self.params['W{}'.format(i+1)], self.params['b{}'.format(i+1)], conv_param)
      hidden_out_list.append(hidden_out)
      hidden_cache_list.append(hidden_cache)
    scores, scores_cache = Linear.forward(hidden_out, self.params['W{}'.format(self.num_layers)], self.params['b{}'.format(self.num_layers)])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dx, grads['W{}'.format(self.num_layers)], grads['b{}'.format(self.num_layers)] = Linear.backward(d1, scores_cache)

    for i in range(self.num_layers-1, 0, -1):
      if i-1 in self.max_pools:
        if self.batchnorm:
          dx, grads['W{}'.format(i)], grads['b{}'.format(i)], grads['gamma{}'.format(i)], grads['beta{}'.format(i)] = Conv_BatchNorm_ReLU_Pool.backward(dx, hidden_cache_list[i-1])
        else:
          dx, grads['W{}'.format(i)], grads['b{}'.format(i)] = Conv_ReLU_Pool.backward(dx, hidden_cache_list[i-1])
      elif self.batchnorm:
        dx, grads['W{}'.format(i)], grads['b{}'.format(i)], grads['gamma{}'.format(i)], grads['beta{}'.format(i)] = Conv_BatchNorm_ReLU.backward(dx, hidden_cache_list[i-1])
      else:
        dx, grads['W{}'.format(i)], grads['b{}'.format(i)] = Conv_ReLU.backward(dx, hidden_cache_list[i-1])
...
def create_convolutional_solver_instance(data_dict, dtype, device):
  ...
  model = DeepConvNet(input_dims=input_dims, num_classes=10, 
                      ...
                      weight_scale='kaiming', batchnorm=True, reg=1e-3, dtype=dtype, device='cuda')
  solver = Solver(model, data_dict, num_epochs=5, batch_size=128,
                  optim_config={
                    'learning_rate': 2e-3,
                  },
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
  ...
  if K is None:
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    temp = torch.ones(1, device=device) * gain / Din
    weight = torch.randn(Din, Dout, dtype=dtype, device=device) * torch.sqrt(temp)
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
  else:
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    temp = torch.ones(1, device=device) * gain / (Din * K * K)
    weight = torch.randn(Dout, Din, K, K, dtype=dtype, device=device) * torch.sqrt(temp)
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
  return weight
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      running_var = momentum * running_var + (1 - momentum) * sample_var
      xmu = (x - sample_mean)
      x_norm = xmu / torch.sqrt(sample_var + eps)
      out = x_norm * gamma + beta
      cache = (x_norm, gamma, xmu, sample_var, eps, mode)
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
    elif mode == 'test':
      ...
      # Replace "pass" statement with your code
      xmu = (x - running_mean)
      x_norm = xmu / torch.sqrt(running_var + eps)
      out = x_norm * gamma + beta
      cache = (x_norm, gamma, xmu, running_var, eps, mode)
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
    else:
  ...
  def backward(dout, cache):
    ...
    # Replace "pass" statement with your code
    N, D = dout.shape
    x_norm, gamma, xmu, var, eps, mode = cache
    ...
    if mode == 'train':
      dx_norm = dout * gamma
      divar = torch.sum(dx_norm * xmu, dim=0)
      dxmu1 = dx_norm * ivar
      dsqrtvar = -1 / (sqrtvar ** 2) * divar 
      dvar = 0.5 * 1 / torch.sqrt(var + eps) * dsqrtvar
      dsq = 1/N * torch.ones_like(dout) * dvar 
      dxmu2 = 2 * xmu * dsq 
      dx1 = dxmu1 + dxmu2 
      dmu = -1 * torch.sum(dxmu1 + dxmu2, dim=0)
      dx2 = 1 / N * torch.ones_like(dout) * dmu 
      dx = dx1 + dx2
    else:
  ...
  def backward_alt(dout, cache):
    ...
    dx_norm = dout * gamma
    if mode == 'train':
      dx = (1. / N) * 1 / torch.sqrt(var + eps) * (N * dx_norm - torch.sum(dx_norm, dim=0)- x_norm*torch.sum(dx_norm*x_norm, dim=0))
    else:
      dx = dout * gamma / torch.sqrt(var + eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
class SpatialBatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    # Replace "pass" statement with your code
    N, C, H, W = x.shape
    x_new = torch.reshape(x.permute(0, 2, 3, 1), (-1, C))
    out, cache = BatchNorm.forward(x_new, gamma, beta, bn_param)
    out = torch.reshape(out, (N, H, W, C)).permute(0, 3, 1, 2).contiguous()
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
  ...
  def backward(dout, cache):
    ...
    # Replace "pass" statement with your code
    N, C, H, W = dout.shape
    dout_new = torch.reshape(dout.permute(0, 2, 3, 1), (-1, C))
    dx, dgamma, dbeta = BatchNorm.backward_alt(dout_new, cache)
    dx = torch.reshape(dx, (N, H, W, C)).permute(0, 3, 1, 2)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
