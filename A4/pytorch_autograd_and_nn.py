...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  fc_w = nn.init.kaiming_normal_(torch.empty(num_classes, channel_2*(W-kernel_size_1-kernel_size_2+8)**2, dtype=dtype, device=device))
...
class ThreeLayerConvNet(nn.Module):
  ...
  def forward(self, x):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    hid_out1 = F.relu(self.conv1(x))
    hid_out2 = F.relu(self.conv2(hid_out1))
    x = flatten(hid_out2)
    scores = self.fc(x)
    ############################################################################
    #                            END OF YOUR CODE                          
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    for arg in stage_args:
      blocks.append(ResNetStage(arg[0], arg[1], arg[2], arg[3], block))
  ...
  def forward(self, x):
    ...
    scores = self.fc(flatten(F.avg_pool2d(x, kernel_size=(xh, xw))))
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
    return scores
